...
type BoothCategoryId = (typeof boothCategories)[number]["id"];

const boothHeroSlides = [
  {
    badge: "10,000개 한정판매",
    background: "#fffbf5",
...
type BoothProduct = (typeof recommendedProducts)[number];
...
function getProductIdFromPathname() {
  const [, productId] = window.location.pathname.match(/^\/booth\/([^/]+)/) ?? [];
...
export function CustomerMapPage() {
  ...
  useEffect(() => {
    const handlePopState = () => {
      setSelectedProductId(getProductIdFromPathname());
    };
